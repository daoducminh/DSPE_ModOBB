...
local function fn(Sim)
    ...
    --emitter:SetLayer(LAYER_WORLD_CEILING)
		...
		if GetWorld().Map ~= nil then
			while inst.num_splashes_to_emit > 0 do
				...
				--if GetWorld().Map:GetTileAtPoint( x, y, z ) ~= GROUND.IMPASSABLE then
                    ...
                    if ncreatedRaindrops < 150 then
                        -- create new raindrop instances
                        local raindrop = SpawnPrefab( "raindrop" )

                        raindrop.Transform:SetPosition( x, y, z )
                        raindrop:Show()
                        raindrop.anim:PlayAnimation( "anim" )

                        raindrop:ListenForEvent( "animover", function(inst) raindrop:Hide() raindrops[#raindrops+1] = raindrop end )

                        ncreatedRaindrops = ncreatedRaindrops + 1
                    else
                        if #raindrops ~= 0 then
                            --take them from the raindrop pool
                            local raindrop = raindrops[#raindrops]
                            table.remove(raindrops,#raindrops)
