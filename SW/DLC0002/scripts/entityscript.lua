...
function EntityScript:SetPrefabName(name)
    ...
    self.name = self.name or (STRINGS.NAMES[string.upper(self.prefab)] or "")
end
...
function EntityScript:ReturnActiveItem()
...
function EntityScript:CancelMenu(bufferedaction)
    if self == GetPlayer() then
        local hover = self.HUD.controls.hover
        if(hover.inMenu and hover.menuFirstAction) then
            if bufferedaction.target == hover.menuFirstAction.target then
                hover:EndMenu()
            end
        end
    end
...
function EntityScript:PushBufferedAction(bufferedaction)
    ...
    -- Cancel menu if the player is going to repeat action
    self:CancelMenu(bufferedaction)
...
function EntityScript:PerformBufferedAction()
    ...
    if self.bufferedaction then
        ...
        -- Cancel menu if the player is going to repeat action
        self:CancelMenu(self.bufferedaction)

        if  success then
            self:PushEvent("actionsuccess", {action = self.bufferedaction})
            if self.bufferedaction and self.bufferedaction.action ~= ACTIONS.PICKUP or not TheInput:IsTouchDown() then
            self:ReturnActiveItem()
end
            self.bufferedaction = nil
            return true
