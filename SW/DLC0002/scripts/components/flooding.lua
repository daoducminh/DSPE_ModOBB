...
--[[
			...
			local tile_type1 = ground.Map:GetTileAtPoint(bx, 0, bz)
			local tile_type2 = ground.Map:GetTileAtPoint(bx + 2, 0, bz)
			local tile_type3 = ground.Map:GetTileAtPoint(bx - 2, 0, bz)

			local tile_type11 = ground.Map:GetTileAtPoint(bx, 0, bz + 2)
			local tile_type12 = ground.Map:GetTileAtPoint(bx + 2, 0, bz + 2)
			local tile_type13 = ground.Map:GetTileAtPoint(bx - 2, 0, bz + 2)

			local tile_type21 = ground.Map:GetTileAtPoint(bx, 0, bz - 2)
			local tile_type22 = ground.Map:GetTileAtPoint(bx + 2, 0, bz - 2)
			local tile_type23 = ground.Map:GetTileAtPoint(bx - 2, 0, bz - 2)


			if (tile_type1 ~= GROUND.LEAKPROOFCARPET and tile_type2 ~= GROUND.LEAKPROOFCARPET and tile_type3 ~= GROUND.LEAKPROOFCARPET and
				tile_type11 ~= GROUND.LEAKPROOFCARPET and tile_type12 ~= GROUND.LEAKPROOFCARPET and tile_type13 ~= GROUND.LEAKPROOFCARPET and
				tile_type21 ~= GROUND.LEAKPROOFCARPET and tile_type22 ~= GROUND.LEAKPROOFCARPET and tile_type23 ~= GROUND.LEAKPROOFCARPET) then
				GetWorld().Flooding:SetIsPositionBlocked(bx,0,bz, false)
				--print("Removing flood block at "..bx..","..bz) 
			--else
			--	print("Not removing flood block at "..bx..","..bz)
			end
]]
