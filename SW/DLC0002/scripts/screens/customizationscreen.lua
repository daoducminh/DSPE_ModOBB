...
local CustomizationScreen = Class(Screen, function(self, profile, cb, defaults, RoGEnabled, merge_options)
	...
	for k = 1, per_side*2 do
      ...
      if APP_REGION == "SCEC" then
        ...
        file = "images/customisation.xml"
			...
			spinner.text:SetRegionSize(120, 60)
			spinner.text:EnableWordWrap(true)
			spinner.background:SetScale(1.0, 1.2)
			local default_value = overrides[options[idx].name] or options[idx].default
...
function CustomizationScreen:VerifyValidSeasonSettings()
	if IsDLCEnabled(CAPY_DLC) then
		local mild = self:GetValueForOption("mild")
		local wet = self:GetValueForOption("wet")
		local green = self:GetValueForOption("green")
		local dry = self:GetValueForOption("dry")
...
function CustomizationScreen:Apply()
	...
	local complex = false
	local oneGigRamDevice = IOS_DEVICE_PHYSICAL_MEMORY <= 1024

	if oneGigRamDevice then
		local local_tweaks = {}
		local numMores = 0
		...
		for k,j in pairs(self.presetspinner:GetSelected().overrides) do
			local m = j[1]
			local n = j[2]
			if n == "often" and (not local_tweaks[m]) then
				numMores = numMores + 1
			end
			if m == "world_size" then
				worldSize = n
		...
		-- complex = (worldSize == "default" and numMores > 40) or (worldSize == "medium" and numMores > 8) or (worldSize == "large" and numMores > 2)
		complex = numMores > 40
	end
