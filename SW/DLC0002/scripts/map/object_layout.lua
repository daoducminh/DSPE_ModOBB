...
local function ReserveAndPlaceLayout(node_id, layout, prefabs, add_entity, position, find_alternative, item)
	assert(node_id~=nil)
	assert(layout~=nil)
	assert(prefabs~=nil)
...
--		end
	...
	if rcx ~= nil then		
		...
		if find_alternative ~= nil then
			if item == nil then
				item = layout.name
			end
			if item ~= 'JungleOasis' then
				find_alternative(node_id,layout,prefabs,add_entity,position,item)
			else
				print("Warning! [NO ALTERNATIVE] Could not find a spot for "..layout.name.." in node "..node_id)
			end
		else
...
local function TryReserveAndPlaceLayout(node_id, layout, prefabs, position)
	assert(node_id~=nil)
	assert(layout~=nil)
	assert(prefabs~=nil)
...
--		end
	...
	local rcy = 0
	if position == nil then
		rcx, rcy = WorldSim:TryReserveSpace(node_id, size, layout.start_mask, layout.fill_mask, layout.layout_position, tiles)
	else
		rcx = position[1]
		rcy = position[2]
	end
...
local function Convert(node_id, item, addEntity, findAlternative)
	...
	ReserveAndPlaceLayout(node_id, layout, prefabs, addEntity, nil, findAlternative, item)
end
...
local function Place(position, item, addEntity, choices)
	...
	local prefabs = ConvertLayoutToEntitylist(layout)
	ReserveAndPlaceLayout("POSITIONED", layout, prefabs, addEntity, position, nil, item)
end


local function TryConvert(node_id, item)
	assert(item and item ~= "", "Must provide a valid layout name, got nothing.")
	local layout = LayoutForDefinition(item)
	local prefabs = ConvertLayoutToEntitylist(layout)
	TryReserveAndPlaceLayout(node_id, layout, prefabs)
end
...
return {
		...
		TryReserveAndPlaceLayout = TryReserveAndPlaceLayout,
	}
