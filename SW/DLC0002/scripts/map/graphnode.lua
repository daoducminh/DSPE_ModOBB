...
-- [Vicent] we need to find an alternative position for the objects if we want to speed up world eneration

-- function Node:ConvertGround(spawnFn, entitiesOut, width, height, world_gen_choices)
function Node:ConvertGround(spawnFn, entitiesOut, width, height, alt_fn)

	-- print(";;; NODE CONVERT GROUND!")
	...
	if self.data.terrain_contents.countstaticlayouts ~= nil then
		for k,count in pairs(self.data.terrain_contents.countstaticlayouts) do
			...
			if layout.water and (self.data.type == nil or self.data.type ~= "water") then
				...
				-- print(";;; END PLACE WATER")
			else
				-- print(";;; PLACE BORDER")
				layout.border = layout.border or border
				for i=1, count do
					obj_layout.ReserveAndPlaceLayout(self.id, layout, prefabs, add_fn, nil, alt_fn)
				end
				-- print(";;; END PLACE BORDER")
			end
		end
	end
	...
	if self.data.terrain_contents_extra and self.data.terrain_contents_extra.static_layouts then
		for i,layoutname in pairs(self.data.terrain_contents_extra.static_layouts) do
			...
			local prefabs = obj_layout.ConvertLayoutToEntitylist(layout)
			if layout.water and (self.data.type == nil or self.data.type ~= "water") then
				-- print(";;; PLACE WATER INNER")
				PlaceWaterLayout(layout, prefabs, add_fn, checkFn)
				-- print(";;; END PLACE WATER INNER")
			else
				-- print(";;; PLACE BORDER INNER")
				layout.border = layout.border or border
				obj_layout.ReserveAndPlaceLayout(self.id, layout, prefabs, add_fn, nil, alt_fn)
				-- print(";;; END PLACE BORDER INNER")
			end
		end
	end
