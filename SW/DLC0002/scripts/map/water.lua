...
local function squareFill(width, height, x, y, radius, ground)
	...
	-- [vicent] optimizations:
	-- * avoid inner if (by simple calculating correct max and min values for the fors)
	-- * inlined "is_waterlined" function

	-- local minX = math.max(0, x - radius)
	-- local maxX = math.min(width-1, x + radius)

	-- local minY = math.max(0, y - radius)
	-- local maxY = math.min(height-1, y + radius)

	-- local t = nil

	-- local coral = GROUND.OCEAN_CORAL
	-- local mangrove = GROUND.MANGROVE

	-- for yy = minY, maxY, 1 do
	-- 	for xx = minX, maxX, 1 do
	-- 		t = WorldSim:GetTile(xx, yy)
	-- 		if not (WorldSim:IsLand(t) or t == coral or t == mangrove) then
	-- 			WorldSim:SetTile(xx, yy, ground)
	-- 		end
	-- 	end
	-- end
...
function ConvertImpassibleToWater(width, height, data)
	...
	local function do_squarefill(shallowRadius, mediumRadius)
		...
		-- [vicent] try to optimize this function by inlining elements and avoid table lookups

		-- local function local_IsCloseToImpassableOrWater(x, y, radius)
		-- 	local xMinusRad = x - radius
		-- 	local xPlusRad = x + radius
		-- 	local minI = (-radius+y)
		-- 	local maxI = (radius+y)
		-- 	local tMinus = nil
		-- 	local tPlus = nil
		-- 	local impassable = GROUND.IMPASSABLE

		-- 	for i = minI, maxI, 1 do
		-- 		tMinus = WorldSim:GetTile(xMinusRad, i)
		-- 		tPlus = WorldSim:GetTile(xPlusRad, i)
		-- 		if tMinus == impassable or tPlus == impassable or WorldSim:IsWater(tMinus) or WorldSim:IsWater(tPlus) then
		-- 			return true
		-- 		end
		-- 	end
		...
		-- 	minI = (-radius-1+x)
		-- 	maxI = (radius-1+x)
		-- 	local yMinusRad = y - radius
		-- 	local yPlusRad = y + radius

		-- 	for i = minI, maxI, 1 do
		-- 		tMinus = WorldSim:GetTile(i, yMinusRad)
		-- 		tPlus = WorldSim:GetTile(i, yPlusRad)
		-- 		if tMinus == impassable or tPlus == impassable or WorldSim:IsWater(tMinus) or WorldSim:IsWater(tPlus) then
		-- 			return true
		-- 		end
		-- 	end
		...
		-- local coral = GROUND.OCEAN_CORAL
		-- local mangrove = GROUND.MANGROVE
		-- local ocean_shallow = GROUND.OCEAN_SHALLOW

		-- for y = 0, height - 1, 1 do
		-- 	for x = 0, width - 1, 1 do
		-- 		local ground = WorldSim:GetTile(x, y)
		-- 		if WorldSim:IsLand(ground) or ground == coral or ground == mangrove then -- is_waterlined(ground)
		-- 			if local_IsCloseToImpassableOrWater(x,y,shallowRadius) then -- (IsCloseToTileType(x, y, shallowRadius, GROUND.IMPASSABLE) or IsCloseToWater(x, y, shallowRadius)) then
		-- 				squareFill(width, height, x, y, shallowRadius, ocean_shallow)
		-- 			end
		-- 		end
		-- 	end
		-- end
