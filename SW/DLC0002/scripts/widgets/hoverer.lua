...
function HoverText:StartMenu(title)
    ...
    if title and IPHONE_VERSION then
        ...
        self.titleText:SetPosition(0,(YOFFSETUPTEXT*(1+items)*factor - 7.5*items),0)
        self.titleText:Show()
    end

    if title then
        self.isWorldMenu = false
    else
        self.isWorldMenu = true
    end

    if IPHONE_VERSION and (not self.isWorldMenu) then
