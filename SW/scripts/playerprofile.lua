...
function PlayerProfile:Reset()
    ...
    --    self.persistdata.sendstats = true
        self.persistdata.controller_popup = false
        elf.persistdata.virtualstick = false
	end
...
function PlayerProfile:GetBatteryModeEnabled()
    if USE_SETTINGS_FILE then
        local value = TheSim:GetSetting("graphics", "batteryMode")
        if value == nil then
            if IPHONE_VERSION then -- Default to true if phone version
                return false
            else
                return false
            end
        ...
        return self:GetValue("batteryMode")
    end
end

function PlayerProfile:SetBatteryModeEnabled(enabled)
    if USE_SETTINGS_FILE then
        TheSim:SetSetting("graphics", "batteryMode", tostring(enabled))
    else
        self:SetValue("batteryMode", enabled)
        self.dirty = true
    end

    -- [blit-vicent] battery mode on Android ??
    -- TheSim:SetBatteryMode(enabled)
end
