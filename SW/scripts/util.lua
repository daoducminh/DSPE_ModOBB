...
--[[
	if PLATFORM == "NACL" or PLATFORM == "PS4" or PLATFORM == "XBONE" then
...
function RunInSandbox(untrusted_code)
	...
	if not untrusted_function then 
        ...
        local str = untrusted_code
        -- savedata = [string "return {playerinfo={x=30.43,data={krakener={a..."]:1403: main function has more than 65536 constants
        local match = string.match(savedata, ":(%d+): main function has more than 65536 constants")
        if match then
            match = string.match(str, "ents%s*=%s*{")
            if match then
                local entsIni = string.find(str, match)
                if entsIni then
                    entsIni = string.find(str, '{', entsIni)
                    local entsEnd = entsIni
                    local left = 0
                    local right = 0
                    local assigns = 0
                    local i = entsIni
                    ...
                    if entsEnd > entsIni then
                        print("ents found from", entsIni, "to", entsEnd, "with", left, "open and closed brackets and", assigns, "assigments")

                        -- Subdivide in groups
                        local groups = {}
                        local remainStr = string.sub(str, entsIni+1, entsEnd-1)
                        local remainAssigns = assigns
                        repeat
                            local group
                            group, remainStr, remainAssigns = SaveGameIndex:SubdivideEnts(remainStr, remainAssigns)
                            if group then
                                table.insert(groups, group)
                            end
                        until remainAssigns < 65536
                        if remainStr then
                            table.insert(groups, remainStr)

                            -- Whole savedata but ents
                            str = string.sub(str, 1, entsIni)..string.sub(str, entsEnd)
                            success, savedata = RunInSandbox(str)
                            if success then
                                for i=1, #groups, 1 do
                                    local success2, savedata2 = RunInSandbox("return {"..groups[i].."}")
                                    if success2 then
                                        for k,v in pairs(savedata2) do
                                            print(k)
                                            savedata.ents[k] = v
                                        end
                                    else
