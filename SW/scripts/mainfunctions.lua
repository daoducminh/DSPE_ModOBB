...
function SavePersistentString(name, data, encode, callback, toiCloud)
	if TheFrontEnd then
        ...
        if toiCloud then 
    		TheSim:SetPersistentString(name, data, encode, function()
                TheSim:UploadToiCloud(cb)
            end)
        else
        ...
        if toiCloud then 
            TheSim:SetPersistentString(name, data, encode, function()
                TheSim:UploadToiCloud(callback)
            end)
        else
...
function SaveGame(savename, cb)
    ...
    -- [blit-vicent] TOREVIEW not used right now
    -- TheSim:ForceUnloadAssets()
    
    --save the entities
    local nument = 0
    local saved_ents = {}
    local references = {}
    TheSim:GCWorker_WaitUntilFinished()
    collectgarbage()
    ...
    -- print("REACHED SAVE POINT")

	save.mods = ModManager:GetModRecords()
    save.super = WasSuUsed()


    -- print("REACHED SAVE POINT 2")

    TheSim:GCWorker_WaitUntilFinished()
    collectgarbage()

    BlitPrintDebugMapInfo(save, "ON_SAVE")

    TheSim:GCWorker_WaitUntilFinished()
    collectgarbage()    

    -- print("REACHED SAVE POINT 3")
    ...
    -- print("REACHED SAVE POINT 4")
    
    -- local data = DataDumper(save, nil, BRANCH ~= "dev")
    local data = DataDumper(save, nil, true, true)
    print("REACHED SAVE POINT 4.5")

    local insz, outsz
    if PLATFORM == "iOS" then
        insz, outsz = SavePersistentString(savename, data, false, callback, TheSim:IsiCloudEnabled())
    else
        insz, outsz = SavePersistentString(savename, data, false, callback, false)
    end
    print ("Saved", savename, outsz)
    
    
    -- print("REACHED SAVE POINT 5")

    if player.HUD then
		player:PushEvent("ontriggersave")
    end

    -- print("REACHED SAVE POINT 6")
    ...
    if isConnected and not sawPopup then
        ...
        local function enableControllers()
          -- set all connected controllers as enabled in the player profile
            ...
            -- [vicent] we don't have dialog to pop on iOS
        ...
        -- [vicent] remove this dialog from iOS
        -- Check_Mods()
