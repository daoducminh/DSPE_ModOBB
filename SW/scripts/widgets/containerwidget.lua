...
local ContainerWidget = Class(Widget, function(self, owner)
    ...
    self.holdTimer = 0
    self.holdFinished = false
end)

function ContainerWidget:OnControl(control, down)
	if IPHONE_VERSION then
		if self.holdTimer < HOLD_MAX_TIME then
			if not down then
				ContainerWidget._base.OnControl(self, control, true)
			else
				return
			end
		end
	...
	if ContainerWidget._base.OnControl(self, control, down) then return true end
...
function ContainerWidget:Open(container, doer)
    ...
    if container.components.container.widgetpos then
		...
		if IPHONE_VERSION then
			local wpos = Vector3(0, 0, 0)
			wpos.x, wpos.y, wpos.z = container.components.container.widgetpos.x, container.components.container.widgetpos.y, container.components.container.widgetpos.z
			wpos.y = wpos.y - 60
			self:SetPosition(wpos)
		end
	end
	...
	if self.isopen and self.owner and self.container then
		...
		if IPHONE_VERSION then
			if TheInput:IsTouchDown() then
				...
				if self.holdTimer >= HOLD_MAX_TIME and not self.holdFinished then
					ContainerWidget._base.OnControl(self, CONTROL_ACCEPT, true)
					self.holdFinished = true
				end 
			else
				self.holdFinished = false
				self.holdTimer = 0
