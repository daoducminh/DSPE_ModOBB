...
local UIClock = Class(Widget, function(self)
    ...
    if usedLanguage == "korean" or usedLanguage == "simplified_chinese" or usedLanguage == "traditional_chinese" or usedLanguage == "japanese" then
        self.text = self:AddChild(Text(DEFAULTFONT, 33/self.base_scale))
        self.text:SetPosition(2, 0/self.base_scale, 0)
    elseif usedLanguage == "italian" then
        self.text = self:AddChild(Text(NUMBERFONT, 28/self.base_scale))
        self.text:SetPosition(2, 0/self.base_scale, 0)
    else
