...
function CraftSlot:OnControl(control, down)
    ...
    if control == CONTROL_ACCEPT and IPHONE_VERSION then
        if not down then
...
--           self.recipepopup:SetPosition(-220,-260,0)
            self.recipepopup:SetScale(0.95, 0.95, 0.95)
            self.recipepopup:SetPosition(-228,-270,0)
        else
            self.recipepopup:SetPosition(0,-20,0)
        end
    end
