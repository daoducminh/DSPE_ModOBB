...
local RecipePopup = Class(Widget, function(self, horizontal)
    ...
    if JapaneseOnPS4() then
        ...
        self.desc:SetPosition(320, -10, 0)
        self.desc:SetRegionSize(64*3+30,70+80)	
    end
    ...
    self.button:SetOnClick(function() if not DoRecipeClick(self.owner, self.recipe) then end self.owner.HUD.controls.crafttabs:Close()  self:Refresh() end)
    ...
    self.teaser:SetPosition(325, -110, 0)
...
function RecipePopup:Refresh()
    ...
    if should_hint then
        ...
        local hint_text = 
        {
            ...
            ["SEALAB"] = STRINGS.UI.CRAFTING.NEEDALCHEMYENGINE,
        }
...
function RecipePopup:AdjustText()
    local textW, textH = self.name:GetRegionSize()
    local buttonW, buttonH = self.bg:GetSize()

    if(textW > buttonW*0.6) then
        local ratio = buttonW / textW
        local currentSize = self.name.size
        self.name:SetSize(currentSize * ratio * 0.6)
    end

    local desc = self.desc:GetString()
    ...
    elseif(len > 35) then
        ...
        self.desc:SetSize(33)
    end
    --print(self.desc:GetString())
end
