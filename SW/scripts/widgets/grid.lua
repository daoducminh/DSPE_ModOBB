...
local Grid = Class(Widget, function(self)
    ...
    self.num_children = 0
    -- Scroll (JGCAM)
    self.scroll = 0 -- first page
    self.maxVisibleColumns = 0 -- num of visible columns
    self.leftArrow = false
    self.rightArrow = false
end)

function Grid:Scroll(direction) -- direction: -1 or 1
	if self.scroll == 0 and direction == -1 then -- left limit
		return
	end
	if (self.cols - self.scroll <= self.maxVisibleColumns) and direction == 1 then -- right limit
		return
	end

	self.scroll = self.scroll + direction -- update the scroll

	self:UpdateScrolling()
end

function Grid:UpdateScrolling()
	-- update the positions and visibility
	for c = 1, self.cols do
		for r = 1, self.rows do
			local item = self:GetItemInSlot(c,r)
			if item then
				item:SetPosition(Vector3(self.h_offset*(c-1-self.scroll), self.v_offset*(r-1), 0))
				if (c - self.scroll) <= 0 then
					item:Hide()
				elseif (c - self.scroll) > self.maxVisibleColumns then
					item:Hide()
				else
					item:Show()
				end
	...
	if self.scroll == 0 and self.leftArrow then
		self.leftArrow:Hide()
	else
		self.leftArrow:Show()
	end

	if (self.cols - self.scroll <= self.maxVisibleColumns) and self.rightArrow then
...
function Grid:InitScrolling(maxVisibleColumns, leftArrow, rightArrow)
	if maxVisibleColumns then
