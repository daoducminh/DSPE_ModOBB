...
SaveIndex = Class(function(self, prefix)
...
function SaveIndex:GetSaveIndexName()
	local name = self.prefix.."saveindex" 
	-- print(" ;;; GET SAVE NAME WITH PREFIX ", name)
	if BRANCH ~= "release" then
		name = name .. "_"..BRANCH
	end
	return name
...
function SaveIndex:Save(callback, indexname, isbackup)
	...
	local data = DataDumper(self.data, nil, true, true)
	if isbackup then
    	local insz, outsz = TheSim:SetPersistentString(indexname, data, false, callback, true)
    else
    	local insz, outsz = TheSim:SetPersistentString(indexname, data, false, callback)
    end
end
...
function SaveIndex:SubdivideEnts(str, assigns)
	...
	while i < string.len(str) do
		...
		elseif string.sub(str,i,i) == '}' then
			right = right + 1
			if left == right and nAssigns > 60000 then
				break;
			end
		elseif string.sub(str,i,i) == '=' then
			nAssigns = nAssigns + 1
	...
	local firtsTryI = i
	local firtsTryAssigns = nAssigns
	if nAssigns >= 65536 then
		print("First search failed with", nAssigns, "assigments")
		i = i - 1
		right = right - 1
		while i > 0 do
			if string.sub(str,i,i) == '{' then
				left = left - 1
			elseif string.sub(str,i,i) == '}' then
				if left == right and nAssigns < 65536 then
					break
				end
				right = right - 1;
			elseif string.sub(str,i,i) == '=' then
	...
	if i > first and nAssigns < 65536 then
		print("slice found with", nAssigns, "assigments")
		local comma = string.find(str, ',', i)

		return string.sub(str, 1, i), string.sub(str, comma+1), assigns - nAssigns
	else
		print("slice NOT found", i, nAssigns)

		if firtsTryI > first then
			local comma = string.find(str, ',', firtsTryI)
			if comma then
				print("return string.sub(str, 1, firtsTryI), string.sub(str, comma+1),", assigns - firtsTryAssigns)
				return string.sub(str, 1, firtsTryI), string.sub(str, comma+1), assigns - firtsTryAssigns
			else
				print("return nil, str,", assigns - firtsTryAssigns)
				return nil, str, assigns - firtsTryAssigns
			end
		end
...
function SaveIndex:GetSaveData(slot, mode, cb, ignoreslot)
	...
	TheSim:GetPersistentString(file, function(load_success, str)
		...
		-- [blit-vicent] remove raindrops from savegame
		savedata.ents["raindrop"] = nil

		BlitPrintDebugMapInfo(savedata, "ON_LOAD")
