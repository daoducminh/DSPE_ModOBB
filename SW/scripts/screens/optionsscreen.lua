...
--		sendstats = Profile:GetSendStatsEnabled(),
	...
	if PLATFORM == "iOS" or PLATFORM == "Android" then
        ...
        self.options.batteryMode = Profile:GetBatteryModeEnabled()
	end
	...
	if show_graphics then
        ...
        if IPHONE_VERSION then
        	if PLATFORM == "Android" then
            	self.grid:InitSize(3, 6, 360, -60)
            else
            	self.grid:InitSize(3, 6, 400, -70)
            end
        else
            self.grid:InitSize(3, 5, 400, -83)
        end
	end
    ...
    if IPHONE_VERSION then
        self.grid:SetPosition(-270, 130, 0)
    end
    ...
    self.rightbutton:SetPosition(540, 0, 0)
    self.rightbutton:SetScale(1.3)
    ...
    self.leftbutton:SetPosition(-540, 0, 0)
    self.leftbutton:SetScale(-1.3,1.3,1.3)
    self.leftbutton:SetOnClick( function() self:Scroll(-1) end)	

    self.grid:InitScrolling(2, self.leftbutton, self.rightbutton)
...
function OptionsScreen:Scroll(direction)
...
--	Profile:SetSendStatsEnabled( self.options.sendstats )
	if PLATFORM == "iOS" or PLATFORM == "Android" then
        ...
        Profile:SetBatteryModeEnabled(self.options.batteryMode)
	end
...
function OptionsScreen:CreateSpinnerGroup( text, spinner, textScale )
	...
	--if PLATFORM == "Android" then
		...
		spacing = 30
	--end
	local label = group:AddChild( Text( BUTTONFONT, size, text ) )
	label:AdjustToWidth(150)
	if textScale then
		textScale = textScale or 1.0
		label:SetScale( textScale )
		label:SetPosition( -(label_width*0.5*textScale) + spacing, 0, 0 )
	else
		label:SetPosition( -label_width/2 + spacing, 0, 0 )
	end
...
function OptionsScreen:UpdateMenu()
	...
	if self:IsDirty() then
		--if IPHONE_VERSION and not TheSim:SupportsRecording() then
			...
			--self.menu:AddItem(STRINGS.UI.OPTIONS.APPLY, function() self:ApplyChanges() end, Vector3(0, 0, 0))
			--self.menu:AddItem(STRINGS.UI.OPTIONS.REVERT, function() self:RevertChanges() end,  Vector3(0, 0, 0))
		--else
...
--[[
	...
	local spinnerTextSize = 35

	self.screenshakeSpinner = Spinner( enableDisableOptions )
    self.screenshakeSpinner.text:SetSize(spinnerTextSize)
    ...
    self.fxVolume.text:SetSize(spinnerTextSize)
    ...
    self.musicVolume.text:SetSize(spinnerTextSize)
    ...
    self.ambientVolume.text:SetSize(spinnerTextSize)
	...
	if PLATFORM == "iOS" or PLATFORM == "Android" then
		if not self.in_game then
			if PLATFORM == "iOS" then
			    ...
			    self.enableiCloud.text:SetSize(spinnerTextSize)
		    ...
		    self.gamma.text:SetSize(spinnerTextSize)
            ...
            if TheSim:SupportsRecording() then
                ...
                self.recordButtonsSpinner.text:SetSize(spinnerTextSize)
            ...
            self.HDSpinner.text:SetSize(spinnerTextSize)
        ...
        self.actionButtonsSpinner.text:SetSize(spinnerTextSize)
        ...
        self.virtualStickSpinner.text:SetSize(spinnerTextSize)
        ...
        self.batteryModeSpinner = Spinner( enableDisableOptions )
        self.batteryModeSpinner.text:SetSize(spinnerTextSize)
        self.batteryModeSpinner.text:SetPosition(0, -4, 0)
        self.batteryModeSpinner.OnChanged =
            function( _, data )
                this.working.batteryMode = data
                --this:Apply()
                self:UpdateMenu()
            end
	end
    ...
    self.hudSize.text:SetSize(spinnerTextSize)
...
--[[
    ...
    self.vibrationSpinner.text:SetSize(spinnerTextSize)
    ...
    self.sendstatsSpinner.text:SetSize(spinnerTextSize)
    ...
    self.wathgrithrfontSpinner.text:SetSize(spinnerTextSize)
...
--		table.insert( left_spinners, { STRINGS.UI.OPTIONS.DISTORTION, self.distortionSpinner } )
		...
		table.insert( right_spinners, { STRINGS.UI.OPTIONS.VIRTUAL_STICK, self.virtualStickSpinner } )
...
if PLATFORM == "iOS" or PLATFORM == "Android" then
		if not self.in_game then
            ...
            if TheSim:SupportsRecording() then
                table.insert( extra_spinners, { STRINGS.UI.OPTIONS.RECORD_BUTTONS, self.recordButtonsSpinner } )
            end
            
		end

		table.insert( extra_spinners, { STRINGS.UI.OPTIONS.ACTION_BUTTONS, self.actionButtonsSpinner } )
        
        -- table.insert( extra_spinners, false )
        table.insert( extra_spinners, { STRINGS.UI.OPTIONS.BATTERY_MODE, self.batteryModeSpinner, 0.95} )
        table.insert( extra_spinners, {STRINGS.UI.OPTIONS.HD, self.HDSpinner } )
end
...
--		end
	...
	for k,v in ipairs(extra_spinners) do
		if v then
			local titem = self:CreateSpinnerGroup(v[1], v[2], v[3])
			self.grid:AddItem(titem, 3, k)
		end
	end
...
--	self.distortionSpinner:SetSelectedIndex( EnabledOptionsIndex( self.working.distortion ) )
	...
	if PLATFORM == "iOS" or PLATFORM == "Android" then
        ...
        self.batteryModeSpinner:SetSelectedIndex( EnabledOptionsIndex( self.working.batteryMode ) )
	end
