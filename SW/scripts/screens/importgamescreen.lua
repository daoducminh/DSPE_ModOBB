...
local iCloudSyncDialog = require "screens/icloudsyncdialog"
require "os"
...
local ImportGameScreen = Class(Screen, function(self, profile)

	Screen._ctor(self, "ImportGameScreen")
	...
	self.showiCloudDialog = true 
	--self:RefreshFiles()  
end)

function ImportGameScreen:OnBecomeActive()

    --TheGameService:AwardAchievement("achievement_1")
    
	self:RefreshFiles()
	ImportGameScreen._base.OnBecomeActive(self)
...
function ImportGameScreen:OnControl(control, down)
...
function ImportGameScreen:RefreshFiles()
	self.menu:Clear()
...
function ImportGameScreen:MakeSaveTile(slotnum)
	
	local widget = Widget("savetile")
	widget.base = widget:AddChild(Widget("base"))
	
	local mode = ImportSaveGameIndex:GetCurrentMode(slotnum)
	local day = ImportSaveGameIndex:GetSlotDay(slotnum)
	local world = ImportSaveGameIndex:GetSlotWorld(slotnum)
	local character = ImportSaveGameIndex:GetSlotCharacter(slotnum)
	local DLC = ImportSaveGameIndex:GetSlotDLC(slotnum)
	...
	if character and mode and CapyDLC then
		if ImportSaveGameIndex:OwnsMode("survival", slotnum) then
			if ImportSaveGameIndex:OwnsMode("shipwrecked", slotnum) then
				if ImportSaveGameIndex:ROGEnabledOnSWSlot(slotnum) then
    ...
    widget.text = widget.base:AddChild(Text(TITLEFONT, 40))
    widget.text:SetPosition(70,0,0)
    ...
    if not mode then
		...
		widget.text:SetHAlign(ANCHOR_MIDDLE)
	...
	elseif mode == "cave" then
		local level = ImportSaveGameIndex:GetCurrentCaveLevel(slotnum)
...
function ImportGameScreen:OnClickTile(slotnum)
	self.last_slotnum = slotnum
	TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/click_move")	


	-- if not ImportSaveGameIndex:GetCurrentMode(slotnum) then
