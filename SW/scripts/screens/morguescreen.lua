...
--    self.root:SetScale(.9)
    ...
    if JapaneseOnPS4() then
        ...
        if usedLanguage == "japanese" then
            factor = 0.5
        end
        self.DAYS_LIVED = self.obits_titles:AddChild(Text(TITLEFONT, font_size * factor))
    end
    ...
    self.DAYS_LIVED:AdjustToWidth(200)
    ...
    self.DECEASED:AdjustToWidth(200)
...
function MorgueScreen:RefreshControls()
	...
	for k = 1, controls_per_screen do
		...
		if self.mogue[idx] then
            ...
            CAUSE:SetString(killed_by:gsub("^%l", string.upper))
            print(CAUSE)
