...
-- local ImportGameScreen = require "screens/importgamescreen"
...
local NewGameScreen = Class(Screen, function(self, slotnum)
    ...
    local hasCrossSave = false

    local yOff = 0
    if hasCrossSave then
    	yOff = 30
    end

    self.title = self.root:AddChild(Text(TITLEFONT, 60))
    self.title:SetPosition( 75, 135 + yOff, 0)
	...
	self.portraitbg:SetPosition(-120, 135 + yOff, 0)	
	...
	self.portrait:SetPosition(-120, 135 + yOff, 0)
  
  	local menuitems = {}
  	if IsDLCInstalled(REIGN_OF_GIANTS) or IsDLCInstalled(CAPY_DLC) then
		local dlc_buttons = {}

        -- [vicent] hardcode SW as default
	...
	--   }
		...
		-- [vicent] support for importing Pocket Edition slots
		if hasCrossSave then
			table.insert(menuitems, {text = "Import PE Slot", cb = function() TheFrontEnd:PushScreen( ImportGameScreen() ) end})
		end
	...
	self.menu:SetPosition(0, 30 + yOff, 0)
