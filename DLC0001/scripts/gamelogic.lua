		...
		if res then
			...
			local resfn = function()
			    ...
			    if wilson.components.resurrectable:DoResurrect() then
                    local hover = wilson.HUD.controls.hover
...
--					end
        ...
        -- [blit-vicent] HACK-FIX: NoGrue requires to re-interpolate the light color after loading tunning overrides!
        GetClock():LongUpdate( 0 )
...
max.task = max:StartThread(function()	max.components.maxwelltalker:DoTalk() print("Number of Entities: "..TheSim:GetNumberOfEntities()) end)
...
local function DoGenerateWorld(saveslot, type_override)
	...
	elseif world_gen_options.level_type == "cave" then
		...
		-- [blit-vicent] HACK to allow caves to read grue custom option
		local tgrue = SaveGameIndex:GetModeData(saveslot,"survival")
		-- print(" ..TGRUE: ", fastdump(tgrue))

		if tgrue.options ~= nil then
			local tweakgrue = nil
			if tgrue.options.tweak ~= nil and tgrue.options.tweak.misc ~= nil and tgrue.options.tweak.misc.grue ~= nil then
				tweakgrue = tgrue.options.tweak.misc.grue
			elseif tgrue.options.preset == "NOSWEAT" then
				tweakgrue = "never"
			end
			if tweakgrue ~= nil then
				world_gen_options["custom_options"] = {tweak={misc={grue=tweakgrue}}}
			end
		end
	end
