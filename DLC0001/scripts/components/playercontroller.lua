...
function PlayerController:StartPlantMode(tile)
	self.plant_mode = true
	self.active_inventory_tile = tile
	self.active_tile_cursor = tile:AddChild(Image( HUD_ATLAS, "slot_select.tex"))
end

function PlayerController:CancelPlantMode()
	self.plant_mode = false
	self.active_inventory_tile:RemoveChild(self.active_tile_cursor)
	self.active_tile_cursor:Kill()
	self.active_inventory_tile = nil
	self.active_tile_cursor = nil
end
				...
				if (guy:HasTag("monster") and has_weapon) or
					...
					force_attack then
						...
						if guy.components.follower and guy.components.follower.leader then
					        if guy.components.follower.leader.name == STRINGS.NAMES.GLOMMERFLOWER or guy.components.follower.leader.name == STRINGS.NAMES.CHESTER_EYEBONE then
					            discard = true
					        end
					    end
					    ...
					    if guy.components.combat and guy.components.combat.target then
					        ...
					        if target ~= GetPlayer() then
					            if target.components.follower and target.components.follower.leader then 
					                ...
					                if leader ~= self.inst and leader.name ~= STRINGS.NAMES.GLOMMERFLOWER and leader.name ~= STRINGS.NAMES.CHESTER_EYEBONE then
					                    discard = true
					                end
					            end
					        end
...
--[[
	...
	if not self.inst.sg:HasStateTag("busy") then
		if self.draggingonground and not Profile:GetVirtualStickEnabled() then
			local pt = TheInput:GetWorldPositionMove()
...
if not (PLATFORM == "iOS" or PLATFORM == "Android") then
		...
		if v ~= self.inst then
			...
			if should_consider then
				...
				if not target_score or score > target_score or not target_action then
					...
					if ((action or v.components.inspectable) and (not target_score or score > target_score)) and (not toExclude or v ~= toExclude) --better real action
...
function PlayerController:UpdateNearTexts()
	...
	for index=i,self.inst.HUD.controls.maxNearObjects do
		...
		self.inst.HUD.controls.nearTexts[i]:SetTarget(nil)
	end
end
    ...
    if ((not self.directwalking) and ((not self.touchStartedOnUI) or (self.touchStartedOnUI and self.inst.HUD.isLastOpenInventory))) then
	    if firstAction or secondAction then
	        if firstAction and not secondAction then
				...
				hover.menuFirstAction = self.LMBaction
				hover.menuSecondAction = secondAction--self.RMBaction
