...
function Temperature:IsFreezing()
	if self.inst.components.health and self.inst.components.health:GetPercent() <= self:GetMinHealthPercent() then
		return false
	end
...
function Temperature:OnUpdate(dt, applyhealthdelta)
	...
	if applyhealthdelta and self.current < 0 and self.inst.components.health and self.inst.components.health:GetPercent() > self:GetMinHealthPercent() then
...
function Temperature:GetMinHealthPercent()
    return TUNING.COLD_MINHEALTH or 0 
end
