...
local W = 68
local SEP = 12 -- Separation between slots
local YSEP = 8
local INTERSEP = 28 -- Separation between groups of 5 slots
...
local Inv = Class(Widget, function(self, owner)
	...
	-- self.actionstring:SetScale(3,3,3)
...
function Inv:OnUpdate(dt)
	...
	if IPHONE_VERSION and TheInput:IsZoomHudEnabled() and not TheInput:ControllerAttached() then
...
if not self.owner.HUD.controls.virtualstick.dragging then
...
function Inv:CloseInventory()
    print("Inventory-CloseInventory")
	...
	if self.open then
		...
		if TheInput:IsTouchDown() and screenPos.y < OPEN_MENU_TOP then --
			...
			if self.scrollState == SCROLL_HOLD then
				...
				--if dragging an item then don't scroll!
