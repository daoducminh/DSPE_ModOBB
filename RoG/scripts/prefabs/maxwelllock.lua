...
local function OnUnlock(inst, key, doer)
    ...
    local body =  nil
    -- [blit-vicent] RU: we need to remove the format in the second body part
    if TheSim:GetUsedLanguage() == "russian" then
        body = STRINGS.UI.UNLOCKMAXWELL.BODY1..STRINGS.CHARACTER_NAMES[character]..STRINGS.UI.UNLOCKMAXWELL.BODY2
    else
        body = STRINGS.UI.UNLOCKMAXWELL.BODY1..STRINGS.CHARACTER_NAMES[character]..string.format(STRINGS.UI.UNLOCKMAXWELL.BODY2, STRINGS.UI.GENDERSTRINGS[GetGenderStrings(character)].TWO)
    end
