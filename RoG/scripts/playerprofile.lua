...
PlayerProfile = Class(function(self)
    self.persistdata = 
    {
 	...
 	if not USE_SETTINGS_FILE then
        ...
        self.persistdata.sendstats = false
        self.persistdata.controller_popup = false
        self.persistdata.gamma = 4
        self.persistdata.actionButtons = true
        self.persistdata.recordButtons = true
        self.persistdata.virtualstick = false
	end
...
function PlayerProfile:Reset()
    ...
    self.persistdata.customizationpresets = {}
 	...
 	if not USE_SETTINGS_FILE then
        ...
        self.persistdata.sendstats = false
        self.persistdata.controller_popup = false
        self.persistdata.virtualstick = true
	end
...
function PlayerProfile:UnlockEverything()
    self.persistdata.xp = 0
	self.persistdata.unlocked_characters = {}
	local characters = {'willow', 'wendy', 'wolfgang', 'wilton', 'wx78', 'wickerbottom', 'wes', 'waxwell', 'woodie', 'pyro', 'wathgrithr', 'webber'}
...
function PlayerProfile:SetGamma(value)
 	if USE_SETTINGS_FILE then
		TheSim:SetSetting("graphics", "gamma", tostring(value)) 
	else
		self:SetValue("gamma", value)
		self.dirty = true
	end
end

function PlayerProfile:GetGamma()
 	if USE_SETTINGS_FILE then
		return TheSim:GetSetting("graphics", "gamma") or 4
	else
		return self:GetValue("gamma") or 4
	end
end

function PlayerProfile:SetHD(value)
 	if USE_SETTINGS_FILE then
		TheSim:SetSetting("graphics", "hd", tostring(value)) 
	else
		self:SetValue("hd", value)
		self.dirty = true
	end
end

function PlayerProfile:GetHD()
 	if USE_SETTINGS_FILE then
		return TheSim:GetSetting("graphics", "hd") or "true"
	else
		return self:GetValue("hd") or "true"
	end
end

function PlayerProfile:SetActionButtonsEnabled(enabled)
    if USE_SETTINGS_FILE then
        TheSim:SetSetting("graphics", "actionButtons", tostring(enabled))
    else
        self:SetValue("actionButtons", enabled)
        self.dirty = true
    end
end

function PlayerProfile:GetActionButtonsEnabled()
    if USE_SETTINGS_FILE then
        return not (TheSim:GetSetting("graphics", "actionButtons") == "false")
    else
        return self:GetValue("actionButtons")
    end
end

function PlayerProfile:SetVirtualStickEnabled(enabled)
    if USE_SETTINGS_FILE then
        TheSim:SetSetting("graphics", "virtualstick", tostring(enabled))
    else
        self:SetValue("virtualstick", enabled)
        self.dirty = true
    end
end

function PlayerProfile:GetVirtualStickEnabled()
	if TheInput:ControllerAttached() then
        return false
    end
    if USE_SETTINGS_FILE then
        if TheInput:IsZoomHudEnabled() then
             return not (TheSim:GetSetting("graphics", "virtualstick") == "false")
        else
             return TheSim:GetSetting("graphics", "virtualstick") == "true"
        end
    else
        return self:GetValue("virtualstick")
    end
end


function PlayerProfile:SetRecordButtonsEnabled(enabled)
    if USE_SETTINGS_FILE then
        TheSim:SetSetting("graphics", "recordButtons", tostring(enabled))
    else
        self:SetValue("recordButtons", enabled)
        self.dirty = true
    end
end

function PlayerProfile:GetRecordButtonsEnabled()
    if USE_SETTINGS_FILE then
        return not (TheSim:GetSetting("graphics", "recordButtons") == "false")
    else
        return self:GetValue("recordButtons")
    end
end
...
function PlayerProfile:SetHaveWarnedCasual()
	if USE_SETTINGS_FILE then
		TheSim:SetSetting("misc", "warnedcasual", "true") 
	else
		self:SetValue("warnedcasual", true)
		self.dirty = true
	end
end

function PlayerProfile:HaveWarnedCasual()
	if USE_SETTINGS_FILE then
		return TheSim:GetSetting("misc", "warnedcasual") == "true"
	else
		return self:GetValue("warnedcasual")
	end
end
...
function PlayerProfile:Save(callback, disableiCloud)
	Print( VERBOSITY.DEBUG, "SAVING" )
    if self.dirty then
        local str = json.encode(self.persistdata)
        local useiCloud = false
        if PLATFORM == "iOS" then
        	useiCloud = not disableiCloud and TheSim:IsiCloudEnabled()
        end
        local insz, outsz = SavePersistentString(self:GetSaveName(), str, ENCODE_SAVES, callback, useiCloud)
    else
		if callback then
			callback(true)
		end
...
function PlayerProfile:Set(str, callback)
	if not str or string.len(str) == 0 then
		...
		if PLATFORM == "iOS" or PLATFORM == "Android" then
			local gamma = self:GetGamma()
			TheSim:SetGamma(gamma)
		end
			...
			local distortion_enabled = GetValueOrDefault( self.persistdata.distortion, true )
 	        ...
 	        if USE_SETTINGS_FILE then
				...
				if self.persistdata.bloom ~= nil and self.persistdata.distortion ~= nil and self.persistdata.HUDSize ~= nil then
					...
					self.persistdata.gamma = 4
                    self:SetActionButtonsEnabled(self.persistdata.actionButtons)
                    self:SetRecordButtonsEnabled(self.persistdata.recordButtons)
					self.dirty = true
				else
