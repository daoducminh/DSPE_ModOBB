...
local SlotDetailsScreen = Class(Screen, function(self, slotnum)
    ...
    if russianText then
	    self.text:SetPosition( 70, 135, 0)
	    self.text:SetRegionSize(255,60)
    else
...
function SlotDetailsScreen:BuildMenu()
    ...
    local len = string.len(self.text:GetString())
    if len > 15 then
    	if TheSim:GetUsedLanguage() == "russian" then
	    	self.text:SetSize(43)
	    else
	    	self.text:SetSize(50)
	    end
    end
...
function SlotDetailsScreen:Continue(force)
	...
	local mode = SaveGameIndex:GetCurrentMode(self.saveslot)
	local file = SaveGameIndex:GetModeData(self.saveslot, mode).file
	if (file and IPHONE_VERSION) and not force then
		TheSim:GetPersistentString(file ,
	        function(load_success, str) 
				local success, savedata = RunInSandbox(str)

				-- JGCAM DETECT HERE	
				if savedata.map.persistdata.seasonmanager then
					local autumnlength = (savedata.map.persistdata.seasonmanager.autumnlength)
					local corrupt_condition = (not self.RoG) and (autumnlength ~= nil)
					if corrupt_condition then
				...
				local nument = 0
				for k,v in pairs(savedata.ents) do
					for i,j in pairs(v) do
						nument = nument + 1
					end
				end
				if nument > 12000 then
