...
function NewGameScreen:SetNoSweatEnabled(enabled)
	self.NoSweat = enabled
end

function NewGameScreen:SetSavedCustomOptions(options, alldlc)
	if self.savedcustomoptions == nil then
		self.savedcustomoptions = {}
	end

	local currentdlc = MAIN_GAME
	local dlcs = {REIGN_OF_GIANTS, MAIN_GAME}
	...
	if alldlc ~= nil then
		for _, dlc in ipairs(dlcs) do
			self.savedcustomoptions[dlc] = options		
		end
	else
	...
	local function onSet(options, dlc)
		...
		if options then
			...
			if options.complex then
				local screen = BigPopupDialogScreen(STRINGS.COMPLEX_WORLD.TITLE, STRINGS.COMPLEX_WORLD.BODY, 
...
--  TODO figure out if we need this.
	-- [blit-vicent] we didn't see this change with NoSweat integration (CRASH!) ==> yes, we need it! :(
...
function NewGameScreen:Start()
	...
	if not self.NoSweat and IsDLCInstalled(REIGN_OF_GIANTS) and self.RoG and xp <= REIGN_OF_GIANTS_DIFFICULTY_WARNING_XP_THRESHOLD and not Profile:HaveWarnedDifficultyRoG() then
		local popup = BigPopupDialogScreen(STRINGS.UI.NEWGAMESCREEN.ROG_WARNING_TITLE, STRINGS.UI.NEWGAMESCREEN.ROG_WARNING_BODY, 
	...
	elseif self.NoSweat and not Profile:HaveWarnedCasual() then
		...
		self.root:Disable()
		TheFrontEnd:Fade(false, 1, function() SaveGameIndex:StartSurvivalMode(self.saveslot, self.character, self.customoptions, onsaved, GetEnabledDLCs(), self.NoSweat) end )
	end
end
	...
	self.RoGbutton.OnControl = function(_, control, down) 
		...
		if control == CONTROL_ACCEPT and not down then
			...
			elseif self.RoG == false then
                ...
                print("NewGameScreen:MakeReignOfGiantsButton()")
				DisableDLC(REIGN_OF_GIANTS)
			end
			return true
		end
