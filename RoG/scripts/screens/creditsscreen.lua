...
local ios_pages = 
{
    ...
    {x=-260,y=0, bg=3, tx=-260,ty=200, title =STRINGS.UI.CREDITS.THANKYOU, build="credits", bank="credits", anim="7", flavour = STRINGS.UI.CREDITS.EXTRA_THANKS_2},    -- STEAM
    {x=-260,y=0, bg=1, tx=-260, ty=200, title =STRINGS.UI.CREDITS.THANKYOU, build="credits", bank="credits", anim="3", flavour = table.concat(STRINGS.UI.CREDITS.ALTGAMES.NAMES, "\n")},    -- ALTGAMES
    {x=0,y=0, bg=1, tx=0,ty=200, title =STRINGS.UI.CREDITS.BLIT.TITLE, build="credits2", bank="credits2", anim="8", flavour = table.concat(STRINGS.UI.CREDITS.BLIT.NAMES, "\n")},    -- Blit
    {x=-260,y=0, bg=2, tx=-260, ty=200, title =STRINGS.UI.CREDITS.LOLLI.TITLE, build="credits", bank="credits", anim="3", flavour = table.concat(STRINGS.UI.CREDITS.LOLLI.NAMES, "\n")},    -- Lollipop
    {x=0,y=180, bg=1, build="credits2", bank="credits2", anim="11", klei=true},      -- KLEI
}
...
local CreditsScreen = Class(Screen, function(self)
    ...
    self.flavourtext = self.center_root:AddChild(Text(TITLEFONT, fontSize))
    self.flavourtext2 = self.center_root:AddChild(Text(TITLEFONT, fontSize))
    self.thankyoutext = self.center_root:AddChild(Text(BODYTEXTFONT, 20))
	...
	if PLATFORM == "iOS" or PLATFORM == "Android" then
        for k,v in ipairs( ios_pages) do
    ...
    self.titletext = self.center_root:AddChild(Text(TITLEFONT, fontSize))
    ...
    if not TheInput:ControllerAttached() then
        ...
        self.OK_button:SetScale(1.4,1.4,1.4)
        ...
        if PLATFORM ~= "PS4" and APP_REGION ~= "SCEC" then
            self.FB_button = self:AddChild(ImageButton())
            self.FB_button:SetScale(1.4,1.4,1.4)
            ...
            self.FB_button:SetPosition( left_pos_x, 95*2, 0)

            self.TWIT_button = self:AddChild(ImageButton())
            self.TWIT_button:SetScale(1.4,1.4,1.4)
            ...
            self.TWIT_button:SetPosition( left_pos_x, 95, 0)

            self.THANKS_button = self:AddChild(ImageButton())
            self.THANKS_button:SetScale(1.4,1.4,1.4)
            ...
            self.THANKS_button:SetPosition( left_pos_x, 95*3, 0)
            ...
            --focus crap
            self.OK_button:SetFocusChangeDir(MOVE_LEFT, self.THANKS_button)
            self.TWIT_button:SetFocusChangeDir(MOVE_RIGHT, self.FB_button)
            self.FB_button:SetFocusChangeDir(MOVE_LEFT, self.TWIT_button)
            self.FB_button:SetFocusChangeDir(MOVE_RIGHT, self.THANKS_button)
            self.THANKS_button:SetFocusChangeDir(MOVE_LEFT, self.FB_button)
            self.THANKS_button:SetFocusChangeDir(MOVE_RIGHT, self.OK_button)
        end
    end
