...
function Resurrector:Resurrect(dude)
    ...
    self.used = not self.reusable
    self.active = self.reusable
    self.penalty = 0
    self.lastdayused = GetClock().numcycles
...
function Resurrector:OnSave()
    ...
    return {used = self.used, active = self.active, penalty = self.penalty, reusable = self.reusable, lastdayused = self.lastdayused}
end
...
function Resurrector:OnLoad(data)
    ...
    self.reusable = data.reusable or self.reusable
    self.lastdayused = data.lastdayused or self.lastdayused
