...
function Sanity:GetVisualPercent(usepenalty)
	return (TUNING.NO_SANITY_VISUAL_EFFECTS and 1) or self:GetPercent(usepenalty)
end
...
function Sanity:OnUpdate(dt)
	local percent = self:GetVisualPercent()
	local speed = easing.outQuad( 1 - percent, 0, .2, 1) 
	self.fxtime = self.fxtime + dt*speed
	
	PostProcessor:SetEffectTime(self.fxtime)
	
	local distortion_value = easing.outQuad( percent, 0, 1, 1) 
