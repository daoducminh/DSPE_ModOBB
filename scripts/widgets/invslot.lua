...
function InvSlot:DoControl(control)
...
function InvSlot:Click(stack_mod)
    ...
    if active_item and not container_item then
        ...
        if can_take_active_item then
            ...
            if (GetTime() - self.controlDownTime) > 0.4 then
        ...
        inventory.activeitemcontainer = nil
        
    elseif container_item and not active_item then
        GetPlayer().components.playercontroller.last_clicked_slot = self
        if stack_mod and container_item.components.stackable and container_item.components.stackable:StackSize() > 1 then
            ...
            inventory.activeitemslotnumber = slot_number
            inventory.activeitemcontainer = container
        ...
        if can_take_active_item then
            ...
            if stacked then
                ...
                if not cant_trade_stack then
                    inventory:RemoveItem(active_item, true)
                    container:RemoveItemBySlot(slot_number)
                    if inventory.activeitemcontainer then						
						if(inventory.activeitemcontainer:CanTakeItemInSlot(container_item, inventory.activeitemslotnumber)) then
							inventory.activeitemcontainer:GiveItem(container_item, inventory.activeitemslotnumber)
							container:GiveItem(active_item, slot_number, nil, true, true)
						else
							inventory.activeitemcontainer:GiveItem(active_item, inventory.activeitemslotnumber)
							container:GiveItem(container_item, slot_number, nil, true, true)
						end
                    else
                        container:GiveItem(active_item, slot_number, nil, true, true)
                        container:GiveItem(container_item)
                    end
                end
            end
