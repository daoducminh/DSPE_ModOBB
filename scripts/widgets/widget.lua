...
function Widget:OnTouchStart(id, x, y)
    if not self.focus then return nil end

    for k,v in pairs (self.children) do
        if v.focus then
            local widget = v:OnTouchStart(id, x, y);
            if widget ~= nil then return widget end     -- Need to return the widget that is going to handle the touchmove and touchend
        end
    end 
...
function Widget:OnTouchMove(id, x, y)
end

function Widget:OnTouchEnd(id)
    return false
end

function Widget:OnTouchCancel(id)
end

function Widget:OnGesture(gesture, value, velocity, state, x0, y0, x1, y1)
    if not self.focus then return false end

    for k,v in pairs (self.children) do
        if v.focus and v:OnGesture(gesture, value, velocity, state, x0, y0, x1, y1) then return true end
    end 
end

function Widget:OnTapGesture(gesture, x, y)
end
...
function Widget:RemoveChild(child)
    --print("Widget:RemoveChild(child)", self, child)
    if child then
...
function Widget:FollowMouse(yOffset)
    if not yOffset then yOffset = 0 end
    if not self.followhandler then
        self.followhandler = TheInput:AddMoveHandler(function(x,y) self:UpdatePosition(x,y+yOffset) end)
