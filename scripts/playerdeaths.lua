...
function PlayerDeaths:Save(callback)
    if self.dirty then
        ...
        if PLATFORM == "iOS" then
			local insz, outsz = SavePersistentString(self:GetSaveName(), str, ENCODE_SAVES, callback, TheSim:IsiCloudEnabled())
		else
			local insz, outsz = SavePersistentString(self:GetSaveName(), str, ENCODE_SAVES, callback, false)
		end
