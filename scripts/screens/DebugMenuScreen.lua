...
local DebugMenuScreen = Class(Screen, function(self)
	...
	if PLATFORM=="iOS" or PLATFORM == "Android" then
		self.text = self:AddChild(Text(BODYTEXTFONT, 22, "blah"))
	else
	...
	if(PLATFORM == "iOS" or PLATFORM == "Android") then
		local x = RESOLUTION_X/2
		local y = RESOLUTION_Y/2
		self.uparrow = self:AddChild(ImageButton("images/ui.xml", "scroll_arrow.tex", "scroll_arrow_over.tex", "scroll_arrow_disabled.tex"))
		self.uparrow:SetOnClick(
			function() 
				self.menu:Up() 
				self.text:SetString(tostring(self.menu))
			end)
	    self.uparrow:SetPosition(x, y+100)
	    self.uparrow:SetRotation(-90)

	    self.downarrow = self:AddChild(ImageButton("images/ui.xml", "scroll_arrow.tex", "scroll_arrow_over.tex", "scroll_arrow_disabled.tex"))
		self.downarrow:SetOnClick(
			function() 
				self.menu:Down() 
				self.text:SetString(tostring(self.menu))
			end)
	    self.downarrow:SetPosition(x, y-100)
	    self.downarrow:SetRotation(90)

	    self.rightarrow = self:AddChild(ImageButton("images/ui.xml", "scroll_arrow.tex", "scroll_arrow_over.tex", "scroll_arrow_disabled.tex"))
		self.rightarrow:SetOnClick(
			function() 
				self.menu:Right() 
				self.text:SetString(tostring(self.menu))
			end)
	    self.rightarrow:SetPosition(x+100, y)
	    self.rightarrow:SetRotation(0)

	    self.leftarrow = self:AddChild(ImageButton("images/ui.xml", "scroll_arrow.tex", "scroll_arrow_over.tex", "scroll_arrow_disabled.tex"))
		self.leftarrow:SetOnClick(
			function() 
				self.menu:Left() 
				self.text:SetString(tostring(self.menu))
			end)
	    self.leftarrow:SetPosition(x-100, y)
	    self.leftarrow:SetRotation(180)

	    self.okButton = self:AddChild(TextButton("images/ui.xml", "blank.tex","blank.tex","blank.tex","blank.tex"))
	    self.okButton:SetOnClick(
			function() 
				self.menu:Accept()
				self.text:SetString(tostring(self.menu))
			end)
	    ...
	    self.okButton:SetTextSize(100)
	    self.okButton:SetTextColour(0.9,0.8,0.6,1)
		self.okButton:SetTextFocusColour(1,1,1,1)
		self.okButton:SetPosition(x, y)

		self.backButton = self:AddChild(TextButton("images/ui.xml", "blank.tex","blank.tex","blank.tex","blank.tex"))
	    self.backButton:SetOnClick(
			function() 
				if(self.menu:Pop() == false) then
					self:Close()
				end
				self.text:SetString(tostring(self.menu))
			end)
	    ...
	    self.backButton:SetTextSize(100)
	    self.backButton:SetTextColour(0.9,0.8,0.6,1)
		self.backButton:SetTextFocusColour(1,1,1,1)
		self.backButton:SetPosition(x, y-200)
	end
...
function DebugMenuScreen:OnControl(control, down)
	...
	if not down then
		...
		elseif control == CONTROL_ACCEPT and PLATFORM ~= "iOS" and PLATFORM ~= "Android" then -- ( blit )
			self.menu:Accept()
		else
			return false
		end
...
function DebugMenuScreen:OnTapGesture(gesture, x, y)
	--if(gesture == GESTURE_OPEN_DEBUG) then
		--self:Close()
		return true
	--end
end
