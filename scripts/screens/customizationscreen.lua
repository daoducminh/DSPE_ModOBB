...
local CustomizationScreen = Class(Screen, function(self, profile, cb, defaults, RoGEnabled)
	...
	if #options == 0 or self.RoGEnabled ~= RoGEnabled then
		...
		for k,v in pairs(customise.GROUP) do
			...
			for kk, vv in pairs(nextgroup.items) do
				local nextitem = vv
				local nextitemname = vv.name -- lang
                local nextitemtext = vv.text -- eng
				for ii, jj in pairs(nextgroup.items) do
					if jj.order < nextitem.order and jj.order == numitem or nextitem.order < numitem then
						nextitem = jj
						nextitemname = jj.name
                        nextitemtext = jj.text
					end
				end
                print(nextitemname)
                print(nextitemtext)
				table.insert(options, {name = nextitemtext, text = nextitemname, image = nextitem.image, options = nextitem.desc or nextgroup.desc, default = nextitem.value, group = nextgroupname})
				numitem = numitem + 1
			end
			numgroup = numgroup + 1
			numitem = 1
    ...
    local right_col = RESOLUTION_X*.25 - 80
    if IPHONE_VERSION then
    	right_col = right_col - 20
    end
	...
	if profilepresets then
		for i, level in pairs(profilepresets) do
			table.insert(self.presets, {text=level.text, data=level.data, desc = level.desc, overrides = level.overrides, basepreset=level.basepreset, isRoG = level.isRoG})
		end
	end
    ...
    self.optionspanel:SetScale(1.15)
    ...
    --if IPHONE_VERSION then
    	self.rightbutton:SetPosition(320, 0, 0)
    --end
    ...
    --if IPHONE_VERSION then
    	self.leftbutton:SetPosition(-320, 0, 0)
    --end
...
function CustomizationScreen:HookupFocusMoves()
	...
	for k = 1, #self.left_spinners do
		...
		if k < 3 then
			self.left_spinners[k]:SetFocusChangeDir(MOVE_LEFT, self.savepresetbutton)
		elseif (k == (#self.left_spinners - 1)) then
			self.left_spinners[k]:SetFocusChangeDir(MOVE_LEFT, self.applybutton)
		elseif (k == (#self.left_spinners)) then
			self.left_spinners[k]:SetFocusChangeDir(MOVE_LEFT, self.cancelbutton)
		else
	...
	self.presetspinner:SetFocusChangeDir(MOVE_DOWN, self.applybutton)
	self.presetspinner:SetFocusChangeDir(MOVE_UP, self.savepresetbutton)

	self.savepresetbutton:SetFocusChangeDir(MOVE_DOWN, self.presetspinner)
	self.savepresetbutton:SetFocusChangeDir(MOVE_RIGHT, self.left_spinners[1])

	self.applybutton:SetFocusChangeDir(MOVE_UP, self.presetspinner)
	self.applybutton:SetFocusChangeDir(MOVE_DOWN, self.cancelbutton)
	self.applybutton:SetFocusChangeDir(MOVE_RIGHT, self.left_spinners[(#self.left_spinners-1)])

	self.cancelbutton:SetFocusChangeDir(MOVE_UP, self.applybutton)
	self.cancelbutton:SetFocusChangeDir(MOVE_RIGHT, self.left_spinners[(#self.left_spinners)])
...
function CustomizationScreen:RefreshOptions()
	...
	for k = 1, per_side*2 do
      ...
      if APP_REGION == "SCEC" then
        file = "images/customisation_tencent.xml"
      else
        file = "images/customisation.xml"
      end
      
			local image = opt:AddChild(Image(options[idx].atlas or file, options[idx].image))
			
			local imscale = .5
			image:SetScale(imscale,imscale,imscale)
		    image:SetTooltip(options[idx].text)
...
function CustomizationScreen:LoadPreset(preset)
	for k,v in pairs(self.presets) do
		if preset == v.data then
			...
			if (not self.RoGEnabled) and v.isRoG then
				print("invalid preset! switching to ".. self.presets[1].text)
                self.preset = self.presets[1]
				self.options.preset = self.presets[1].data
				self:RefreshOptions()
...
function CustomizationScreen:SavePreset()
	...
	local function AddPreset(index, presetdata)
        ...
        -- JGCAM here goes the flag!!
        if self.RoGEnabled then
            print("setting preset as RoG")
        	preset.isRoG = true
        end
	...
	if presetnum > self.max_num_presets then
		...
		label:SetPosition( -220/2 - 85, 0, 0 )
		label:SetRegionSize( 220, 50 )
		label:SetHAlign( ANCHOR_MIDDLE )
		local usedLanguage = TheSim:GetUsedLanguage()
		if usedLanguage == "german" then
			label:SetHAlign( ANCHOR_RIGHT ) 
			label:AdjustToWidth(145)
		elseif usedLanguage == "italian" or usedLanguage == "spanish" or usedLanguage == "portuguese" then
			label:AdjustToWidth(180)
		end
		local menuitems = 
	    {
...
function CustomizationScreen:Apply()
	...
	local function collectCustomPresetOptions()
		...
		if self.invalidPreset then
			self.presetspinner:SetSelectedIndex(1)
		end
	...
	if IPHONE_VERSION then
		local numMores = 0
		local worldSize = "default"
		for k,j in pairs(self.options.tweak) do
			for m,n in pairs(j) do
				if n == "often" then
					numMores = numMores + 1
				end
				if m == "world_size" then
					worldSize = n
				end
			end
		end
		for k,j in pairs(self.presetspinner:GetSelected().overrides) do
			for m,n in pairs(j) do
				if n == "often" then
					numMores = numMores + 1
				end
		...
		print("Total Mores: "..numMores)
		print("World Size: "..worldSize)
		complex = (worldSize == "default" and numMores > 15) or (worldSize == "medium" and numMores > 8) or (worldSize == "large" and numMores > 2)
	end

	self.options.complex = complex
	...
	if IsDLCEnabled(REIGN_OF_GIANTS) then
		...
		--if self.invalidPreset then
		--	TheFrontEnd:PushScreen(PopupDialogScreen(STRINGS.UI.CUSTOMIZATIONSCREEN.INVALIDDLCCOMBO_TITLE, STRINGS.UI.CUSTOMIZATIONSCREEN.INVALIDDLCCOMBO_BODY, 
		--				{{text=STRINGS.UI.CUSTOMIZATIONSCREEN.OKAY, cb = function() TheFrontEnd:PopScreen() end}}))
		--else
			collectCustomPresetOptions()
			self.cb(self.options)
