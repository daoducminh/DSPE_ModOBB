...
local DeathScreen = Class(Screen, function(self, days_survived, start_xp, mode, capped, cb)
    ...
    if capped then
        self:Capped(days_survived, start_xp, mode)
    else
        self:NotCapped(days_survived, start_xp, mode)
    end
end)

function DeathScreen:NotCapped(days_survived, start_xp, mode)
    local factor = 1
    if IPHONE_VERSION then
        factor = 1.5
    end

    local russianText = TheSim:GetUsedLanguage() == "russian"
    ...
    elseif russianText then
        -- 
        self.t1 = self.root:AddChild(Text(font, 35*factor))
        self.t1:SetHAlign(ANCHOR_LEFT)
        self.t1:SetPosition(-140*factor,55*factor,0)
        self.t1:SetString(STRINGS.UI.DEATHSCREEN.SURVIVEDDAYS) --"You survived days: ..."


        self.survivedtext = self.root:AddChild(Text(font, 40*factor)) -- Day Count
        self.survivedtext:SetPosition(20*factor,55*factor,0)
    ...
    if russianText then
        ...
        self.leveltext:SetPosition(-160*factor,3*factor,0)
        ...
        self.rewardtext = self.root:AddChild(Text(font, 25*factor)) --"Next Reward..."
        self.rewardtext:SetString(STRINGS.UI.DEATHSCREEN.NEXTREWARD)
        self.rewardtext:SetHAlign(ANCHOR_LEFT)
        self.rewardtext:SetPosition(-10*factor,-45*factor,0)
    else
        self.leveltext = self.root:AddChild(Text(font, 35*factor)) --Level #: 
        self.leveltext:SetHAlign(ANCHOR_LEFT)
    ...
    local menu_items = {}
    if mode == "escaped" then
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.MAINMENU, cb = function() self:OnMenu(true) end})
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.CONTINUE, cb = function() self:OnContinue() end})
    elseif mode == "nosweat" then
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.CONTINUE, cb = function() self:OnClose() end})
    else
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.MAINMENU, cb = function() self:OnMenu(false) end})
...
function DeathScreen:Capped(days_survived, start_xp, mode)
    local factor = 1
    if IPHONE_VERSION then
        factor = 1.5
    end
...
function DeathScreen:ShowResults(days_survived, start_xp)
    ...
    if self.thread then
        ...
        if days_survived == 1 then
            if self.t2 ~= nil then -- [blit-vicent] we don't have this in russian
