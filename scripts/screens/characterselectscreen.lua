...
local CharacterSelectScreen = Class(Screen, function(self, profile, cb, no_backbutton, default_character)
    ...
    if IPHONE_VERSION then
        self.biobox:SetPosition(822 + adjust,RESOLUTION_Y-489+35,0)
        self.biobox:SetScale(1.25, 1.2, 1.2)
    else
        self.biobox:SetPosition(822 + adjust,RESOLUTION_Y-489+30,0)
    end
    
    self.charactername = self.fixed_root:AddChild(Text(TITLEFONT, 62))
    self.charactername:SetHAlign(ANCHOR_MIDDLE)
    if IPHONE_VERSION then
        self.charactername:SetPosition(820 + adjust, RESOLUTION_Y - 400+55,0)
    else
    ...
    if IPHONE_VERSION then
        self.characterquote = self.fixed_root:AddChild(Text(UIFONT, 36))
    else
    ...
    if IPHONE_VERSION then
        self.characterquote:SetPosition(820 + adjust, RESOLUTION_Y - 525 + 60+50,0)
        self.characterquote:SetRegionSize( 650, 60 )
    else
    ...
    if IPHONE_VERSION then
        self.characterdetails = self.fixed_root:AddChild(Text(BODYTEXTFONT, 36))
    else
    ...
    if IPHONE_VERSION then
        self.characterdetails:SetPosition(820 + adjust, RESOLUTION_Y - 525 - 30+45,0)
        self.characterdetails:SetRegionSize( 550, 140 )
    else
    ...
    if not TheInput:ControllerAttached() then
        ...
        if IPHONE_VERSION then
            self.startbutton:SetPosition( 820 + 105 + adjust, 50, 0)
            self.startbutton:SetScale( 1.2)
        else
        ...
        if IPHONE_VERSION then
            self.randomcharbutton:SetPosition( 820 - 98 + adjust, 50, 0)
            self.randomcharbutton:SetScale( 1.2)
        else
		...
		if not no_backbutton then
            ...
            if IPHONE_VERSION then
                self.startbutton:SetPosition( 820 + 205+ adjust, 50, 0)
                self.randomcharbutton:SetPosition( 820 + 5 + adjust, 50, 0)
            else
            ...
            if IPHONE_VERSION then
                self.backbutton:SetPosition( 820 - 195+ adjust, 50, 0)
                self.backbutton:SetScale( 1.2)
            else
    ...
    if IPHONE_VERSION then
        self.rightbutton:SetPosition(1129+10, RESOLUTION_Y-181, 0)
    else
        self.rightbutton:SetPosition(1129+10, RESOLUTION_Y-211, 0)
    end
    self.rightbutton:SetOnClick( function() self:Scroll(1) self:GetCharacterForPortrait(self.portrait_idx) self:SelectPortrait(self.portrait_idx) end)
    ...
    if IPHONE_VERSION then
        self.leftbutton:SetPosition(516+15-10, RESOLUTION_Y-181, 0)
    else
        self.leftbutton:SetPosition(516+15-10, RESOLUTION_Y-211, 0)
    end
    self.leftbutton:SetScale(-1,1,1)
    self.leftbutton:SetOnClick( function() self:Scroll(-1) self:GetCharacterForPortrait(self.portrait_idx) self:SelectPortrait(self.portrait_idx) end)
	...
	if down then
		if dir == MOVE_LEFT then
			if self.navigating == true then
				...
				if self.inrandombutton == true and not self.no_cancel then
					self.backbutton:SetFocus()
					self.inrandombutton = false
					self.inbackbutton = true
				elseif self.instartbutton == true then
					self.randomcharbutton:SetFocus()
					self.instartbutton = false
					self.inrandombutton = true
				end
			end
		elseif dir == MOVE_RIGHT then
			if self.navigating == true then
				...
				if self.inrandombutton == true and self.can_accept == true then
					self.startbutton:SetFocus()
					self.inrandombutton = false
					self.instartbutton = true
				elseif self.inbackbutton == true then
					self.randomcharbutton:SetFocus()
					self.inbackbutton = false
					self.inrandombutton = true
				end
			end
		elseif dir == MOVE_DOWN then
			self.navigating = false
			if self.navigating == false then
				self.randomcharbutton:SetFocus()
				self.inrandombutton = true
			end
		elseif dir == MOVE_UP then
			...
			if self.navigating == true then
				...
				self.instartbutton = false
				self.backbutton:ClearFocus()
				self.inbackbutton = false
				self.randomcharbutton:ClearFocus()
				self.inrandombutton = false
			end
		end
	end
...
function CharacterSelectScreen:SelectPortrait(portrait)
	...
	if character and self.profile:IsCharacterUnlocked(character) then
      ...
      if APP_REGION == "SCEC" then
        self.heroportait:SetTexture("bigportraits/"..character.."_tencent.xml", character.."_tencent.tex")
      else
		...
		if APP_REGION == "SCEC" then
			self.heroportait:SetTexture("bigportraits/locked_tencent.xml", "locked_tencent.tex")
		else
