...
local GameModeScreen = require "screens/gamemodescreen"
...
local LoadGameScreen = Class(Screen, function(self, profile)
    ...
    local russianIPhone = TheSim:GetUsedLanguage() == "russian" -- and IPHONE_VERSION

	if JapaneseOnPS4() or APP_REGION == "SCEC" then
        self.title = self.root:AddChild(Text(TITLEFONT, 60 * 0.8))
	else
        self.title = self.root:AddChild(Text(TITLEFONT, 60))
	end

	if russianIPhone then
    	self.title:SetPosition( 0, 215, 0)
	    self.title:SetRegionSize(300,70)
	else
		self.title:SetPosition( 0, 215, 0)
...
function LoadGameScreen:OnBecomeActive()
	...
	if PLATFORM == "iOS" then
		if self.showiCloudDialog and TheSim:IsiCloudEnabled() then
			TheFrontEnd:PushScreen( iCloudSyncDialog() )
			self.showiCloudDialog = false
		end
	end
end
...
function LoadGameScreen:MakeSaveTile(slotnum)
	...
	if TheSim:GetUsedLanguage() == "japanese" then
    	widget.text = widget.base:AddChild(Text(TITLEFONT, 40 * 0.8))	-- KAJ
	elseif russianIPhone then
		widget.text = widget.base:AddChild(Text(TITLEFONT, 40 * 0.9))	-- KAJ
	else
    ...
    -- widget.text = widget.base:AddChild(Text(TITLEFONT, 40))
    if russianIPhone then
	    widget.text:SetPosition(60,0,0)
	    widget.text:SetRegionSize(210 ,70)
	else
...
function LoadGameScreen:OnClickTile(slotnum)
	...
	if not SaveGameIndex:GetCurrentMode(slotnum) then
		TheFrontEnd:PushScreen(GameModeScreen(slotnum))
	else
		local DLC = SaveGameIndex:GetSlotDLC(slotnum)
		local RoG = (DLC ~= nil and DLC.REIGN_OF_GIANTS ~= nil) and DLC.REIGN_OF_GIANTS or false
		if RoG == true then
		...
		elseif RoG == false then
            print("LoadGameScreen:OnClickTile")
