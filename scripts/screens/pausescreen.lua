...
local PauseScreen = Class(Screen, function(self)
	...
	self.bg:SetScale(1.5*factor,1.2*factor,1.2*factor)
	
	--title	
    self.title = self.proot:AddChild(Text(TITLEFONT, 50*factor))
    self.title:SetPosition(0, 50*factor, 0)
    if IsGamePurchased() or GetTimePlaying() < TUNING.DEMO_TIME then
    	self.title:SetString(STRINGS.UI.PAUSEMENU.TITLE)
	else
		self.title:SetString("DEMO OVER")
	end
	...
	self.menu:SetPosition(-(button_w*(#buttons-1))/2*factor, -65, 0)
