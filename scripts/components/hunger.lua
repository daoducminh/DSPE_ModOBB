...
function Hunger:GetMinHealthPercent()
    return TUNING.HUNGER_MINHEALTH or 0 
end
...
function Hunger:IsStarving() 
    return self.current <= 0 and self.inst.components.health:GetPercent() > self:GetMinHealthPercent()
end
    ...
    if self.burning then
        if self.current <= 0 then
            if not ignore_damage then
                if self.inst.components.health:GetPercent() > self:GetMinHealthPercent() then
